...
const ToFeedButton = () => {
  ...
  const subrepositPath = getSubrepositPath(pathname)

  return (
    <a href={subrepositPath} className={buttonVariants({ variant: 'ghost' })}>
      <ChevronLeft className='h-4 w-4 mr-1' />
      {subrepositPath === '/' ? 'Back home' : 'Back to community'}
    </a>
  )
}

const getSubrepositPath = (pathname: string) => {
